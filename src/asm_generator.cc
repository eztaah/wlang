...
void generate_assembly_internal(const NodePtr &node)
{
    ...
    if (ProgramNode *pnode = dynamic_cast<ProgramNode *>(node.get())) {
        ...
        start_function_instructions.push_back("    subq	$8, %rsp");     // on réalligne la pile sur 16 octets
        ...
        std::string var_offset = "box+" + std::to_string(abs(offset)) + "(%rip)";
