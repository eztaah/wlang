  ...
  if (tokenIndex < tokens.size()) {
    ...
    if (expected_type != UNDEFINED && current_token.first != expected_type) {
      throw std::runtime_error("Expected token type " +
                               tokenTypeToString(expected_type) + ", got (" +
                               tokenTypeToString(current_token.first) + ", " +
...
NodePtr parse_factor()
{
  ...
  while (tokenIndex < tokens.size() && (tokens[tokenIndex].first == TIMES ||
                                        tokens[tokenIndex].first == DIVIDE)) {
  ...
  if (ProgramNode *pnode = dynamic_cast<ProgramNode *>(node.get())) {
    ...
    if (!inode->_true_block.empty()) {
      ...
      for (size_t i = 0; i < inode->_true_block.size(); ++i) {
        print_ast(inode->_true_block[i], output, next_indent + "   ",
                  i == inode->_true_block.size() - 1);
      }
    } else {
      output += next_indent + "└── true_block: " + "\n";
    }
    ...
    output += indent + branch +
              "NumberNode(value=" + std::to_string(nnode->_value) + ")" + "\n";
