...
generate_assembly(const NodePtr &node,
                  std::unordered_map<std::string, std::string> &variables,
                  int offset, int label_count)
{
  ...
  if (ProgramNode *pnode = dynamic_cast<ProgramNode *>(node.get())) {
    ...
    for (const NodePtr &stmt : pnode->_statements) {
      auto [stmt_instructions, new_label_count] =
          generate_assembly(stmt, variables, offset, label_count);
      instructions.insert(instructions.end(), stmt_instructions.begin(),
                          stmt_instructions.end());
      offset -= 8;
    ...
    auto [value_instructions, new_label_count] =
        generate_assembly(vnode->_value, variables, offset, label_count);
    instructions.insert(instructions.end(), value_instructions.begin(),
                        value_instructions.end());
    ...
    auto [right_instructions, right_label_count] =
        generate_assembly(bnode->_right, variables, offset, label_count);
    instructions.insert(instructions.end(), right_instructions.begin(),
                        right_instructions.end());
    ...
    instructions.insert(instructions.end(), left_instructions.begin(),
    ...
    auto [condition_instructions, cond_label_count] =
        generate_assembly(inode->_condition, variables, offset, label_count);
    instructions.insert(instructions.end(), condition_instructions.begin(),
                        condition_instructions.end());
    ...
    for (const NodePtr &stmt : inode->_true_block) {
      auto [true_block_instructions, block_label_count] =
          generate_assembly(stmt, variables, offset, cond_label_count + 1);
      instructions.insert(instructions.end(), true_block_instructions.begin(),
                          true_block_instructions.end());
