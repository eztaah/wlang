...
static Void asme_instr(AsmG* asmg, InstrNode* instr_node);
...
static Void asme_fundef(AsmG* asmg, const FunDefNode* fundef_node)
{
    ...
    asmg->rbp_offset = -8; // TODO: Pourquoi pas 0 ?
...
static Void asme_instr(AsmG* asmg, InstrNode* instr_node)
{
    switch (instr_node->type) {
        case NODE_VAR_DEF:
            asme_vardef(asmg, &instr_node->var_def);
            break;
