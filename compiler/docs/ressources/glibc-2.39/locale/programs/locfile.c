...
/* When called outside a start_locale_structure/end_locale_structure
   or start_locale_prologue/end_locale_prologue block, record that the
...
/* Start building data that goes before the next element's recorded offset.
   ...
   end_locale_prologue switches back to the usual behavior.  */
void
start_locale_prologue (struct locale_file *file)
{
  assert (file->structure_stage == 0);
  file->structure_stage = 3;
}

/* End a block started by start_locale_prologue.  */
void
end_locale_prologue (struct locale_file *file)
{
